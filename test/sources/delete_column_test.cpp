...
TEST(matrices_tests, delete_column_test)
{
  // clang-format off
  auto a = matrix_type(1.f, 2.f, 3.f,
                       ...
                       7.f, 8.f, 9.f);
  // clang-format on
  using T = delete_column_view<matrix_type, 1>;
  auto t = T(a);
  STATIC_ASSERT(T::traits_type::number_of_rows == 3);
  STATIC_ASSERT(T::traits_type::number_of_columns == 2);
  STATIC_ASSERT(is_matrix<T>::value);
  ASSERT_TRUE(t(0, 0) == 1.f);
  ASSERT_TRUE(t(0, 1) == 3.f);
  ASSERT_TRUE(t(1, 0) == 4.f);
  ASSERT_TRUE(t(1, 1) == 6.f);
  ASSERT_TRUE(t(2, 0) == 7.f);
  ASSERT_TRUE(t(2, 1) == 9.f);
}
